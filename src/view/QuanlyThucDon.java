...
//    private final ArrayList<DishesButton> listMonAn = new ArrayList<>();
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Ngừng sử dụng", "Sử dụng" }));
...
//        searchMonAn_jPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 0, 0));
        ...
        searchMonAn_jButton1.setPreferredSize(new Dimension(40, 35)); 
...
//        quanLyThucDon_pane_bg.add(quanLyThucDon_jPanel);
    ...
    public void showMonAnTuDataBase() {
        try {
            ...
            ResultSet monAnResultSet = ExcuteSQLStatement.ExcuteSQLQuery(sqlStatement,connection);
            while (monAnResultSet.next()) {
                ...
                while (maNguyenLieuResultSet.next()) {
                    ...
                    ResultSet tenNguyenLieuResultSet = ExcuteSQLStatement.ExcuteSQLQuery(sqlStatementTenNguyenLieu,connection);
                ...
                ResultSet tenMonAnResultSet = ExcuteSQLStatement.ExcuteSQLQuery(sqlStatement1,connection);
                while (tenMonAnResultSet.next()) {
                    String tenloaiMonAn = tenMonAnResultSet.getString("TENLMA");
                    DishesButton newDishesButton = new DishesButton(link_image, tenMon, tenloaiMonAn, maMonAn, donGia, nguyenLieuDaChon,100,50);
...
//                    listMonAn.add(newDishesButton);
    ...
    private void confirmThemMonAn_jButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confirmThemMonAn_jButtonActionPerformed
        ...
        try {
            while (loaiMonAnResultSet.next()) {
                ...
                String sqlStatementUpdateMonAn = "insert into MONAN values('" + maMonAn + "','" + tenMon + "','" + maLoaiMonAn + "'," + donGia + ",'" + link_img + "')";
        ...
        if (success == true) {
            DishesButton newDishesButton = new DishesButton(link_img, tenMon, tenLoaiMonAn, maMonAn, donGia, nguyenLieuDaChon, 100,50);
            for (JPanel loaiMonAnJPanel : loaiMonAnJPanels) {
                if (newDishesButton.getTenLoaiMonAn().equals(loaiMonAnJPanel.getName())) {
                    loaiMonAnJPanel.add(newDishesButton);
                }
    ...
    private void confirmThemLoaiMonAn_jButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confirmThemLoaiMonAn_jButtonActionPerformed
        ...
        ExcuteSQLStatement.ExcuteSQLUpdate("insert into LOAIMONAN values ('" + maLoaiMonAn + "', '" + themTenLoaiMonAn_jTextField.getText() + "', '" + themMoTaLoaiMonAn_jTextArea.getText() + "')",connection);
    ...
    private void comfirmSuaLoaiMonAn_jButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_comfirmSuaLoaiMonAn_jButtonActionPerformed
        ...
        try {
            while (maLoaiMonAnResultSet.next()) {
                ExcuteSQLStatement.ExcuteSQLUpdate("update LOAIMONAN set TENLMA = '" + tenLoaiMonAnMoi + "' where MALMA = '" + maLoaiMonAnResultSet.getString("MALMA") + "'",connection);
                ExcuteSQLStatement.ExcuteSQLUpdate("update LOAIMONAN set MOTA = '" + moTaLoaiMonAnMoi + "' where MALMA = '" + maLoaiMonAnResultSet.getString("MALMA") + "'",connection);
            }
        } catch (SQLException ex) {
            Logger.getLogger(QuanlyThucDon.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//    private javax.swing.JButton themMonAn_jButton;    
    ...
    private JComboBox tinhTrangLoaiMonAn_jComboBox;
    private JLabel tinhTrangLoaiMonAn_jLabel;
